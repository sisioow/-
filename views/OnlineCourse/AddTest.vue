<template>
  <div class="add-test">
    ...
    <el-table :data="tests" style="width: 100%">
      <el-table-column prop="title" label="测试题目"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    fetchUnits() {
      axios.get('http://10.71.6.132:8080/unit/findUnitBySemesterId?semester_id=1')
        .then(response => {
          this.units = response.data.data; // 将从后端获取的Paper数据赋给papers数组
    ...
    onSubmit() {
      ...
      if (this.form.unityid && this.form.name && this.duration) {
        ...
        // 发送POST请求保存数据到数据库
        axios.post('http://10.71.6.132:8080/unitTest/setUnitTest',data)
          .then(response => {
            // 成功保存后将数据添加到tests数组中
            this.fetchTests();
            this.form.unityid = '';
    ...
    fetchPapers() {
      axios.get('http://10.71.6.132:8080/paperStore/getPaperStore')
        .then(response => {
    ...
    axios.get(`http://10.71.6.132:8080/unitTest/getUnitTestById?unit_id=${this.selectedUnity}`)
      .then(response => {
    ...
    deleteTest(test) {
      ...
      if (index !== -1) {
        // 发送删除请求到后端（如果需要）
        axios.delete(`http://10.71.6.132:8080/unitTest/deleteUnitTest?unit_test_id=${test.unit_test_id}`)
          .then(() => {
